...
class GnnNet1(MetaTemplate):
  ...
  def forward_gnn_refine(self, zs):
    ...
    # scores_mean = torch.where(scores>mean,scores,zero)


    # max = scores.max(dim=2,keepdim=True)[0]
    # one = torch.ones_like(scores)
    # scores_q_s_label = torch.where(scores < max, zero, one)

    medium = torch.zeros_like(nodes)
    for i in range(self.n_way):
      medium[:,2*i:2*i+1,:] = nodes[:,2*i+1:2*i+2,:]
      medium[:,2*i+1:2*i+2,:] = nodes[:,2*i:2*i+1,:]
      medium[:,2*i:2*i+1,128:] = scores[:,2*i+1:2*i+2,:]
      medium[:,2*i+1:2*i+2,128:] = zero[:,2*i+1:2*i+2,:]
    scores1 = self.gnn1(medium)
  ...
  def forward_gnn_refine1(self, zs):
    ...
    #mean
    # zero = torch.zeros_like(scores1)
    # mean = scores1.mean(dim=2,keepdim=True)
    # scores1_mean = torch.where(scores1>mean,scores1,zero)

    # zero = torch.zeros_like(scores1)
    # max = scores1.max(dim=2,keepdim=True)[0]
    # one = torch.ones_like(scores1)
    # scores_s_q_label = torch.where(scores1 < max, zero, one)

    for u in range(self.n_way):
      nodes[:, 2 * u:2 * u + 1, 128:] = scores1[:, 2 * u + 1:2 * u + 2,:]
    scores = self.gnn(nodes)  # 16*10*5
